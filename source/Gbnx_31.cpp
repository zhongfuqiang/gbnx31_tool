// Gbnx_31.cpp: implementation of the CGbnx_31 class.
//
//////////////////////////////////////////////////////////////////////
...
const sMcmdInfor gmc_cmdinfor[] =
{
    ...
    // 组3   pn:测量点号
    {MCMD_AFN_4_F17_TML_UP_CFG,       MC_DIR_M2S, MC_PN_MP,  emc_trans_afn04f17,    "监测终端上行通信参数配置"},
...
eMcErr emc_trans_afn04f6(eMcTrans eTrans, void* pUser, void* pFrame, UINT16* pusLen)
{
    ...
    if(MC_TRANS_U2F == eTrans)
    {
        ...
        if(pu->bCasc == TRUE)
        {
            ...
            if(pu->ucN > 3)
            {
                ...
                #ifdef MC_DBG
                MCDBG("emc_trans_afn04f6() pu->ucN is out of range %d!", pu->ucN);
                #endif
                return MC_ERR_OUTRNG;
            }
        ...
        for(i = 0; i < pu->ucN; i++)
        {
            ...
            // sIp
            eRet = emc_trans_ip(eTrans, &(pu->sOne[i].sIp), &sIp_f, &usLenTmp);
            if(MC_OK != eRet)
            {
                #ifdef MC_DBG
    ...
    else if(MC_TRANS_F2U == eTrans)
    {
        ...
        pu->ucN      = (pf->ucN & 0x7F);
        pu->bCasc = (pf->ucN & 0x80) ? TRUE : FALSE;
    ...
    if(MC_TRANS_U2F == eTrans)
    {
        ...
        switch(pu->eAcsPort)
        {
            ...
            case MC_ACS_PORT_ZIGB:
                pf->eAcsPort = 5;      
                pf->ucAcsParaLen = 8; 
                memcpy(pTmp, pu->uPortPara.mac, 8);
                break;
    ...
    else
    {
        ...
        switch(pf->eAcsPort)
        {
            ...
            case 5:
                ...
                memcpy((void*)&(pu->uPortPara.mac[0]), pTmp, 8);
                break;
...
{
    return emc_trans_afn04f2(eTrans, pUser, pFrame, pusLen);
}
...
{  
    ...
    if(MC_TRANS_U2F == eTrans)
    {
        ...
        for(i = 0; i < ucN; i++)
        {
	        ...
	        memcpy(&(pf->sOne[i]) , &sDownCfg_f, sizeof(sMcDownCfg_f));
    ...
    else if(MC_TRANS_F2U == eTrans)
    {
	 ...
	 ucN = pu->ucN;
        ...
        for(i = 0; i < ucN; i++)
        {
	        memcpy(&sDownCfg_f, &(pf->sOne[i]) , sizeof(sMcDownCfg_f));
...
************************************************************************/
BOOL CGbnx_31::CStringToHexBuffer(CString strText, char *pOutBuf, UINT16 *pLen)
{
	if(!pOutBuf || !pLen)
	{
		return FALSE;
...
CString CGbnx_31::GetAfnName(eMcAFN eAfn)
{
	CString strValue;
	switch (eAfn)
	{
	case MC_AFN_00_CONF:
		strValue = "AFN_00_CONF 确认否认";
		break;
		
	case MC_AFN_01_RSET:
		strValue = "MC_AFN_01_RSET 复位";
		break;
		
	case MC_AFN_04_SETP:
		strValue = "MC_AFN_04_SETP 设置参数";
		break;
		
	case MC_AFN_05_CTRL:
		strValue = "MC_AFN_05_CTRL 控制命令";
		break;
		
	case MC_AFN_06_AUTH:
		strValue = "MC_AFN_06_AUTH 身份认证及密钥协商";
		break;
		
	case MC_AFN_08_CASC:
		strValue = "MC_AFN_08_CASC 请求被级联终端主动上报";
		break;
		
	case MC_AFN_10_GETP:
		strValue = "MC_AFN_10_GETP 查询参数";
		break;
		
	case MC_AFN_12_ASKT:
		strValue = "MC_AFN_12_ASKT 请求（定时上报）任务数据";
		break;
		
	case MC_AFN_13_ASKR:
		strValue = "MC_AFN_13_ASKR 请求实时数据";
		break;
		
	case MC_AFN_14_ASKE:
		strValue = "MC_AFN_14_ASKE 请求（主动上报）事件记录";
		break;
		
	case MC_AFN_15_FILE:
		strValue = "MC_AFN_15_FILE 文件传输";
		break;
		
	case MC_AFN_16_DATA:
		strValue = "MC_AFN_16_DATA 数据转发";
		break;
...
char* CGbnx_31::getErrMessage(eMcErr err)
{
	char * pRet;
	switch(err)
	{
	case MC_OK:             // 无错误   
		pRet = "MC_OK";
		break;
	case MC_ERR_UNCOMP:           // 指针为空    
        pRet = "MC_ERR_UNCOMP";
		break;
	case MC_ERR_NONE:        // 参数越界  
		pRet = "MC_ERR_NONE";
		break;
	case MC_ERR_NULL:           // 不存在:没有找到  
		pRet = "MC_ERR_NULL";
		break;
	case MC_ERR_OUTRNG:         // IO错误    
		pRet = "MC_ERR_OUTRNG";
		break;
	case MC_ERR_IO:        // 资源不足
		pRet = "MC_ERR_IO";
		break;
	case MC_ERR_RES:       // 没有初始化
		pRet = "MC_ERR_RES";
		break;
	case MC_ERR_INIT:       // 参数错误
		pRet = "MC_ERR_INIT";
		break;
	case MC_ERR_PARA:         // 指定Afn 中不支持的 Fn
		pRet = "MC_ERR_PARA";
		break;
	case MC_ERR_FN:         // 指定Afn Fn 中不支持的 Pn
		pRet = "MC_ERR_FN";
		break;
	case MC_ERR_PN:       // 无效地址
		pRet = "MC_ERR_PN";
		break;
	case MC_ERR_ADDR:       // 控制域错误 
		pRet = "MC_ERR_ADDR";
		break;
	case MC_ERR_CTRL:        // SEQ域错误
		pRet = "MC_ERR_CTRL";
		break;
	case MC_ERR_0x10:     // 
		pRet = "MC_ERR_0x10";
		break;
	case MC_ERR_A2CS:        // AFN 错误
		pRet = "MC_ERR_A2CS";
		break;
	case MC_ERR_AFN:     // 不完整的协议包
		pRet = "MC_ERR_AFN";
		break;

	case MC_ERR_0x68:      // 错误的协议标识
		pRet = "MC_ERR_0x68";
		break;
	case MC_ERR_PROTO:         // 错误的检测和
		pRet = "MC_ERR_PROTO";
		break;
	case MC_ERR_CS:       // 错误的结尾符
		pRet = "MC_ERR_CS";
		break;
	case MC_ERR_0x16:      // 错误的处理逻辑
		pRet = "MC_ERR_0x16";
		break;
	case MC_ERR_LOGIC:       // 不是一个有效的包 
		pRet = "MC_ERR_LOGIC";
		break;

	case MC_ERR_PACK:     // 不完整的协议包
		pRet = "MC_ERR_PACK";
		break;
	case MC_ERR_PFC:       // 错误的报文起始
		pRet = "MC_ERR_PFC";
		break;
	case MC_ERR_TIMEOUT:      // 错误的协议标识
		pRet = "MC_ERR_TIMEOUT";
		break;
	case MC_ERR_PRM:         // 错误的检测和
		pRet = "MC_ERR_PRM";
		break;
	case MC_ERR_TEAM:       // 错误的结尾符
		pRet = "MC_ERR_TEAM";
		break;
	case MC_ERR_ENCRYPT:      // 错误的处理逻辑
		pRet = "MC_ERR_ENCRYPT";
		break;
	case MC_ERR_DECRYPT:       // 不是一个有效的包 
		pRet = "MC_ERR_DECRYPT";
		break;

	case MC_ERR_L1L2:       // 错误的结尾符
		pRet = "MC_ERR_L1L2";
		break;
	case MC_ERR_FCODE:      // 错误的处理逻辑
		pRet = "MC_ERR_FCODE";
		break;
	case MC_ERR_DIR:       // 不是一个有效的包 
		pRet = "MC_ERR_DIR";
		break;
...
CString CGbnx_31::BufferToHex(char *pBuf, UINT16 usLen)
{
	CString strOut;
	CString strTmp;
	int len;
