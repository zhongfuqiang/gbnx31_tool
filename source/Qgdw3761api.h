 ...
 ******************************************************************************
  ...
  1.日    期   : 2013年7月25日 星期四
    作    者   : liming(email: vliming@126.com)
    修改内容   : 创建文件
...
//#define MT_USER_MAX         (1024*MT_SEQ2CS_BYTES_MAX)          // 用户侧等变长结构 smtPack smtBasePack 最大长度
...
typedef enum
{
   ...
   /**********************************
    ...
    CMD_AFN_D_F98_CURVE_BACK_HAVE    = 0x0D63,       // 反向有功总电能量 曲线
...
int    bMtGetFloatSign(float fVal);              // 取得一个flaot的符号位  sign = -1 / +1
void   vMtSetFloatSign(float *pfVal, int sign);  // 设置一个浮点数的符号位 sign = -1 / +1
///*}
...
UINT16 usmt_get_aux_len(eMtAFN eAFN, eMtDir eDir, BOOL bEc, BOOL bTp);  // 获得报文中三个域的帧侧的总字节数据
///*}
...
#define MT_USER_CLASS_MIN        (1)  // 用户大类名最小值 
#define MT_USER_CLASS_MAX        (16) // 用户大类名最大值
...
{    
    ...
    sMtAsk1CfgOne  sOne[MT_ASK_M_MAX];   // M个信息
...
typedef struct
{
    ...
    UINT8         ucN;      // 数据单元标识个数n
    sMtCmdPn      sCmdPn[MT_FN_MAX];   // N个数据单元标识  
...
typedef struct
{
    ...
    BOOL           bBig[MT_USER_CLASS_MAX];    // 支持的用户大类号标志
                                               // 依次分别对应第0～15号用户大类，置"1"表示支持，置"0"表示不支持，
                                               // 且对应的"支持用户小类号个数"应为0。
    UINT8          ucSmall[MT_USER_CLASS_MAX]; // 支持0 ~ 15号用户大类下的用户小类号个数
...
typedef struct 
{
    BOOL bParaStatus[MT_TERMINAL_STATUS_USER_MAX];  // 终端参数状态
...
typedef struct
{
    eMtCmd eCmd;                  // 有疑问: 是不是只是设置参数的那些CMD 还是支持所有命令?
    UINT16 ucPn;
...
typedef union
{
  /***************************************
    ...
    sMtAfn0cF03      sTmlParaStat;     // 终端参数状态            CMD_AFN_C_F3_TML_PARA_STATE 
    sMtAfn0cF04      sTmlUpComStat;    // 终端上行通信状态        CMD_AFN_C_F4_TML_UPCOM_STATE 
    ...
    sMtAfn0cF57      sTmlCurtHarmValue; // 当前A、B、C三相电压、电流2～N次谐波有效值CMD_AFN_C_F57_CURT_HARM_VALUE
    sMtAfn0cF58      sTmlCurtHarmRate;  // 当前A、B、C三相电压、电流2～N次谐波含有率CMD_AFN_C_F58_CURT_HARM_RATE
    sMtAfn0cf129     sTmlFrthHavePowrC; // 当前正向有功电能示值（总、费率1～M）CMD_AFN_C_F129_FRTH_HAVE_POWR_C
    sMtAfn0cf130     sTmlFrthNonePowrC; // 当前正向无功（组合无功1）电能示值（总、费率1～M）CMD_AFN_C_F130_FRTH_NONE_POWR_C
    sMtAfn0cf166     sTmlMetrModifyRecd; // 电能表参数修改次数及时间 CMD_AFN_C_F166_METR_MODIFY_RECD
    sMtAfn0cf167     sTmlMetrBuyUse;     // 电能表购、用电信息CMD_AFN_C_F167_METR_BUY_USE
    ///*}
