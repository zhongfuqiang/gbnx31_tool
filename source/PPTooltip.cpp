...
{
	m_pParentWnd = NULL;
    m_hIcon = NULL;   /*GDI对象泄露的原因，不停loadImage*/
	m_memDC.m_hDC = NULL;
	m_memBitmap.m_hObject = NULL;
...
//	CDC * pDC = GetDC();
	...
	//of the tooltip
	CSize sz = PrintTitleString(&m_memDC, rect, str);
...
HICON CPPToolTip::GetIconFromResources(UINT nID, CSize szIcon /* = CSize(0, 0) */) /*const*/
{
	...
	if (NULL == m_hIcon)
	{
		m_hIcon = (HICON)::LoadImage(hInstResource, MAKEINTRESOURCE(nID), IMAGE_ICON, szIcon.cx, szIcon.cy, 0);
	}
	return m_hIcon;
}
...
void CPPToolTip::AddTool(CWnd * pWnd, CString sTooltipText, UINT nIdIcon, CSize szIcon /* = CSize(0, 0) */,
						 LPCRECT lpRectTool /* = NULL */, UINT nIDTool /* = 0 */)
{
	HICON hIcon	= /*NULL;//*/GetIconFromResources (nIdIcon, szIcon);
