...
CCJColorPopup::CCJColorPopup( CPoint p, COLORREF crColor, COLORREF crDefaultBkColor, CWnd* pParentWnd, LPCTSTR szDefaultText /*= NULL*/, LPCTSTR szCustomText /*= NULL*/ )
{
	Initialise();

	m_crColor       = m_crInitialColor = crColor;
	m_pParent        = pParentWnd;
	m_crBkDefault   = crDefaultBkColor;
...
{
	// added by liming
	m_crBkDefault = ::GetSysColor(COLOR_3DLIGHT);
...
// 	}
	...
	{
		m_crColor = m_crBkDefault;
		m_pParent->SendMessage(CPN_SELCHANGE, (WPARAM) m_crBkDefault, 0);
	}
	else
	{
...
void CCJColorPopup::DrawCell(CDC* pDC, int nIndex)
{
	...
	if (m_strDefaultText.GetLength() && nIndex == DEFAULT_BOX_VALUE)
	{
		...
		//pDC->FillSolidRect(m_DefaultTextRect, m_crBkDefault);
		...
		pDC->FillSolidRect(TextButtonRect, ::GetSysColor(COLOR_3DFACE));
		//pDC->FillSolidRect(TextButtonRect, m_crBkDefault);
		...
		CPen pen(PS_SOLID, 1, m_crBkDefault);
