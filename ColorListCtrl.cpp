...
int CColorListCtrl::AddItem( LPCTSTR pszText, ... )
{
	...
	CString stradd;
	stradd=" ";
	stradd=stradd+pszText;
	const int iIndex = InsertItem( GetItemCount(), stradd );
	...
	for( int iColumn = 1; iColumn < m_iNumColumns; iColumn++ )
	{
		...
		stradd=" ";
		stradd=stradd+pszText;
		VERIFY( CListCtrl::SetItem( iIndex, iColumn, LVIF_TEXT, stradd, 0, 0, 0, 0 ) );
...
void CColorListCtrl::FreeItemMemory( const int iItem )
{
	...
	//LPTSTR* arrpsz = pid->arrpsz;         /*原代码,修改为下面代码*/
	LPTSTR* arrpsz =NULL;                   /*防止为空的情况,2009.11.13*/
    if (NULL != pid && pid->arrpsz)
    {
		arrpsz = pid->arrpsz;
    }
	else
